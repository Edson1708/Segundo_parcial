...
public class MainController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        int id, pos;
        if (ses.getAttribute("almacen") == null) {
            ArrayList<Producto> almacenaux = new ArrayList<Producto>();
            ses.setAttribute("almacen", almacenaux);
        }
        ArrayList<Producto> listaalmacen = (ArrayList<Producto>) ses.getAttribute("almacen");
        String op = request.getParameter("op");
        String opcion = (op != null) ? op : "view";
        Producto obj1 = new Producto();
        switch (opcion) {
            case "nuevo":
                request.setAttribute("Producto1", obj1);
                request.getRequestDispatcher("editar.jsp").forward(request, response);
                break;
            case "editar":
                id = Integer.parseInt(request.getParameter("id"));
                pos = buscarIndice(request, id);
                obj1 = listaalmacen.get(pos);
                request.setAttribute("Producto1", obj1);
                request.getRequestDispatcher("editar.jsp").forward(request, response);
                break;
            case "eliminar":
                id = Integer.parseInt(request.getParameter("id"));
                pos = buscarIndice(request, id);
                listaalmacen.remove(pos);
                response.sendRedirect("index.jsp");
                break;
            case "view":
                //ses.setAttribute("almacen", listaalmacen);
                response.sendRedirect("index.jsp");
                break;
        }
    }
